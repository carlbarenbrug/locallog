...
struct ContentView: View {
    ...
    private let autoGeneratedFilenameCharacterLimit = 30
    ...
    private func commitTitleEdit() {
        ...
        if let index = entries.firstIndex(where: { $0.id == entryId }) {
            renameEntryFiles(at: index, to: trimmed.isEmpty ? "untitled" : trimmed, applyAutoGeneratedLimit: false)
        }
    ...
    private func syncAutoGeneratedTitleForSelectedEntry(from oldContent: String, to newContent: String) {
        ...
        let previousLimitedAutoBaseName = sanitizeStoredBaseName(
            autoGeneratedTitle(for: oldContent),
            maximumLength: autoGeneratedFilenameCharacterLimit
        )

        // Only keep auto-renaming while the filename still reflects the generated title.
        guard currentBaseName == previousAutoBaseName || currentBaseName == previousLimitedAutoBaseName else { return }

        let nextAutoTitle = autoGeneratedTitle(for: newContent)
        if sanitizeStoredBaseName(nextAutoTitle, maximumLength: autoGeneratedFilenameCharacterLimit) != currentBaseName {
            renameEntryFiles(at: index, to: nextAutoTitle, applyAutoGeneratedLimit: true)
        }
    }
    ...
    private func sanitizeStoredBaseName(_ title: String, maximumLength: Int? = nil) -> String {
        ...
        if let maximumLength {
            cleaned = String(cleaned.prefix(maximumLength))
    ...
    private func renameEntryFiles(at index: Int, to title: String, applyAutoGeneratedLimit: Bool) {
        let baseName = sanitizeStoredBaseName(
            title,
            maximumLength: applyAutoGeneratedLimit ? autoGeneratedFilenameCharacterLimit : nil
        )
        let entry = entries[index]
